% Post-Classification Comparison (PCC) Pipeline Figure
% This figure shows the two-stage approach: classify first, then compare
...
\begin{tikzpicture}[node distance=2.5cm, auto]
    ...
    \node[imagebox, right=of clf_t1] (map_t1) {
        \includegraphics[width=2.5cm]{MScTemplate/figs/input_images/landcover_t1_2018_schematic.png}\\[2pt]
    ...
    \node[imagebox, right=of clf_t2] (map_t2) {
        \includegraphics[width=2.5cm]{MScTemplate/figs/input_images/landcover_t2_2025_schematic.png}\\[2pt]
...
\caption{Post-classification comparison (PCC) pipeline for change detection. Images from two time periods (2018 and 2025) are independently classified into land-cover maps, which are then compared to identify changes. This two-stage approach processes each temporal image separately before comparing the classification results.}
\label{fig:pcc_pipeline}
\end{figure}
