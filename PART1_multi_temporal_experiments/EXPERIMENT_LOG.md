...
| LSTM-1×1     | `exp004_v2`                 | 2-layer ConvLSTM (1×1 kernel)   | 7   | 31.3 M | 2.3 (RQ2f)     |
...
### Files
- Training: `PART1_multi_temporal_experiments/outputs/experiments/exp001_lstm_unet_annual_seed42/`
...
- Analysis: `PART1_multi_temporal_experiments/exp001_planning/PILOT_RESULTS_ANALYSIS.md`
...
Parameters: 31.3M (vs 54.4M for 3×3 kernel — 1×1 kernel dramatically reduces ConvLSTM params)
Data Source: Sentinel-2
Resolution: 10m
Time Steps: 7 (annual, 2018-2024)
Bands per Step: 9 (blue, green, red, R1, R2, R3, nir, swir1, swir2)
...
| Comparison | Metric | Paired Mean Δ (pp) | p_adj | Significant |
|------------|--------|---------------------|-------|-------------|
| LSTM-7 vs LSTM-2 | IoU | +5.43 | 0.030 | ✅ Yes |
| LSTM-7 vs LSTM-14 | IoU | +5.41 | 0.026 | ✅ Yes |
| LSTM-7 vs LSTM-2 | BF@2 | +7.86 | 0.023 | ✅ Yes |
| LSTM-7 vs LSTM-14 | BF@2 | +7.13 | 0.023 | ✅ Yes |
| 3×3 vs 1×1 kernel | IoU | +0.39 | 0.855 | ❌ No |
| All T=7 models | IoU | — | all >0.5 | ❌ No (indistinguishable) |

*Note: Paired per-tile permutation tests with Holm–Bonferroni correction (m varies by family). See report Results section for full tables.*

### Conclusions
1. **Annual sampling (T=7) is optimal**: Best val IoU (58.38%) and test IoU (63.9%)
2. **Higher temporal granularity does not help**: LSTM-14 (54.12%) significantly underperforms LSTM-7
3. **ConvLSTM recurrence provides no benefit over simpler fusion** at either T=2 or T=7
4. **Kernel size doesn't matter**: 1×1 matches 3×3 (p=0.855)
...
### Hypothesis
LSTM-UNet (exp003, 54.65% IoU) should outperform early-fusion U-Net because:
...
### Notes
- Lowest of the T=2 baselines (50.81% vs 54.82% LateFusion, 54.65% LSTM-2)
...
Parameters: ~31.1M (no ConvLSTM recurrence, just 1×1 conv fusion)
Batch Size: 8
Epochs: 400
Loss: Focal Loss (α=0.25, γ=2.0)
Optimizer: AdamW (lr=0.01, weight_decay=5e-4)
...
- Very close to LSTM-2 (54.65%) — recurrence adds little over simple concat at T=2
...
| Component | LSTM-UNet (exp010) | Late-Fusion Pool (exp007) |
|-----------|-------------------|--------------------------|
| Encoder | Shared ResNet-50 | Shared ResNet-50 |
...
### Hypothesis
If LSTM-UNet (exp010) outperforms this pooling baseline, recurrence captures meaningful temporal ordering at T=7.
...
### Notes
- Matches LSTM-7 (58.38%) almost exactly — simple pooling suffices at T=7
...
### Notes
- Outperforms full LSTM-2 (56.27% vs 54.65%) with ~44% fewer parameters
...
Encoder: ResNet-50 (ImageNet pretrained, shared across timesteps)
Input Shape: (B=4, T=7, C=9, H=64, W=64)
ConvLSTM: 1 layer, hidden_dim=32 (vs 2 layers, 256 in exp010)
ConvLSTM Kernel Size: 3x3
Skip Aggregation: max
...
## Analysis: Temporal Contribution (Post-hoc)

**Date**: 2026-02-17
**Research Question**: Why does annual sampling (T=7) outperform both T=2 and T=14?
**Status**: ✅ Complete
**SLURM Job**: 24047272 (idun-07-09, A100 80GB, ~3 min)

### Motivation
The RQ1 results establish THAT annual sampling outperforms, but not WHY. Three candidate mechanisms:
1. **Temporal signal**: Intermediate years carry useful change trajectory info (explains T=7 > T=2)
2. **Input redundancy**: Quarterly observations introduce seasonal variation without adding discriminative content (explains T=7 > T=14)
3. **Overfitting**: T=14's higher input volume causes overfitting on 45 samples (explains T=7 > T=14)

### Experiments Run

#### Experiment A: Input Redundancy (Temporal Autocorrelation)
- Pairwise Pearson correlation between consecutive frames, averaged across 45 OOF tiles and bands
- **T=14 intra-year** (Q2→Q3): r = 0.857 ± 0.012
- **T=14 inter-year** (Q3→Q2'): r = 0.865 ± 0.017
- **T=7 consecutive** (annual): r = 0.945 ± 0.008
- **Key finding**: Intra-year and inter-year correlations are nearly identical in T=14, meaning Q2 and Q3 carry largely redundant information. Annual compositing (averaging Q2+Q3) retains the inter-year change signal while halving sequence length.

#### Experiment B: Training Dynamics
- Train-val IoU gap at best checkpoint epoch:
  - **LSTM-7** (T=7): 0.301 (smallest gap)
  - **LSTM-2** (T=2): 0.312
  - **LSTM-14** (T=14): 0.328 (largest gap)
- Best epoch: T=2: 117±43, T=7: 208±99, T=14: 181±66
- **Key finding**: T=14 shows the largest train-val gap, directionally consistent with overfitting under longer sequences. Differences are modest and confounded by training budget differences.

#### Experiment C: Temporal Gradient Attribution
- Per-timestep gradient importance for LSTM-7: importance_t = mean(|∂L/∂x_t|)
- U-shaped pattern: endpoints (2018: 0.163, 2024: 0.165) dominate over interior years (~0.134)
- Ratio endpoint/interior: 1.22
- **Key finding**: Methodologically debatable (gradient magnitude ≠ information content). NOT included in report per decision to focus on stronger evidence.

### Report Integration
- **Results.tex**: New `\subsubsection{Temporal Contribution Analysis}` with autocorrelation and training dynamics paragraphs + 2 figures
- **Discussion.tex**: New `\subsubsection{Why Does Annual Sampling Outperform?}` interpreting the mechanisms
- Gradient attribution results omitted from report (available in output files for reference)

### Files
- Analysis script: `scripts/analysis/temporal_importance_analysis.py`
- SLURM script: `scripts/slurm/temporal_importance.sh`
- SLURM log: `outputs/logs/slurm_temporal_importance_24047272.log`
- Figures: `outputs/analysis/temporal_contribution/temporal_autocorrelation.{pdf,png}`
- Figures: `outputs/analysis/temporal_contribution/training_dynamics_comparison.{pdf,png}`
- Figures: `outputs/analysis/temporal_contribution/temporal_gradient_importance.{pdf,png}`
- Data: `outputs/analysis/temporal_contribution/autocorrelation_data.npz`
- Data: `outputs/analysis/temporal_contribution/gradient_importance_data.npz`
- Data: `outputs/analysis/temporal_contribution/training_dynamics_summary.json`
...
|------------|-------------|----------------------|
| exp010 vs exp007 (Pool T=7) | Recurrence helps at T=7 | Simple pooling suffices at T=7 |
| exp010 vs exp008 (3D Conv T=7) | Recurrence > learned 3D filtering | 3D conv captures temporal patterns without recurrence |
...
**RQ1 — Temporal sampling**: How does observation density affect land-take detection?
→ Compare bi-temporal (T=2) vs annual (T=7) vs bi-seasonal (T=14) using same LSTM-UNet architecture

**RQ2 — Architecture and fusion**: Does ConvLSTM recurrence improve over simpler fusion?
→ RQ2a: EarlyFusion vs LSTM-2 (T=2)
→ RQ2b: LateFusion vs LSTM-2 (T=2)
→ RQ2c: Pool-7 vs LSTM-7 (T=7)
→ RQ2d: Conv3D-7 vs LSTM-7 (T=7)
→ RQ2e: LSTM-lite vs full LSTM (capacity control, T=2 and T=7)
→ RQ2f: 1×1 vs 3×3 ConvLSTM kernel (T=7)
...
| 2026-02-17 | analysis | Temporal contribution analysis | Autocorrelation, training dynamics, gradient attribution |
| 2026-02-17 | all | Log accuracy audit | Fixed param counts (exp004: 54.1→31.3M, exp005: ~25→32.6M, exp006: ~27→31.1M, exp008: ~35→32.9M), stale IoU refs (53.29→54.65%), stale exp001→exp010 refs, filled TBDs in statistical summary, updated RQ descriptions |
