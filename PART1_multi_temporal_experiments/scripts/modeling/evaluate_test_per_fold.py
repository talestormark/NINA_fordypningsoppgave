...
Output:
    - ``outputs_v3/{experiment}_fold{k}/test_per_tile_iou.csv`` per fold.
...
def evaluate_test_fold(model, val_loader, device):
    model.eval()
    metrics = Metrics()
    per_sample = {}

    with torch.no_grad():
        for batch in tqdm(val_loader, desc="Evaluating test"):
...
def main():
    ...
    per_fold_per_tile = {}
    per_fold_test_mean_iou = {}
    per_fold_aggregated = {}

    for fold in folds:
        print(f"\n{'='*60}\nTest per-fold inference: {args.experiment} fold {fold}\n{'='*60}")
        ...
        print(f"  Building test dataloader (fold {fold})...")
        ...
        print(f"  Test samples in fold {fold}: {len(val_loader.dataset)}")

        results, per_sample = evaluate_test_fold(model, val_loader, device)
        ...
        per_fold_test_mean_iou[fold] = fold_mean_iou
        ...
        csv_path = exp_dir / "test_per_tile_iou.csv"
    ...
    if len(per_fold_test_mean_iou) > 1:
        per_fold_values = [per_fold_test_mean_iou[f] for f in sorted(per_fold_test_mean_iou.keys())]
        test_per_fold_mean = float(np.mean(per_fold_values))
        test_per_fold_std = float(np.std(per_fold_values))

        print(f"\n{'='*60}\nTest (macro, per-fold) summary for {args.experiment}\n{'='*60}")
        print(f"  Per-fold mean per-tile IoU: {[f'{v*100:.2f}%' for v in per_fold_values]}")
        print(f"  Test (macro, per-fold) mean ± std: {test_per_fold_mean*100:.2f}% ± {test_per_fold_std*100:.2f}%")

        summary_dir = V3_OUTPUTS_DIR / args.experiment
        summary_dir.mkdir(parents=True, exist_ok=True)
        summary = {
            "experiment": args.experiment,
            "test_per_fold_mean": test_per_fold_mean,
            "test_per_fold_std": test_per_fold_std,
            "per_fold_test_mean_iou": per_fold_test_mean_iou,
            "per_fold_aggregated_micro": {
                str(k): {kk: float(vv) for kk, vv in v.items()}
                for k, v in per_fold_aggregated.items()
            },
        }
        summary_path = summary_dir / "test_per_fold_summary.json"
