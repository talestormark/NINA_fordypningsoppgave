    ...
    for SAMPLING in "${SAMPLINGS[@]}"; do
        ...
        for FOLD in $(seq 0 $(($NUM_FOLDS - 1))); do
            ...
            JOB_ID=$(sbatch --parsable \
                PART1_multi_temporal_experiments/scripts/slurm/train_lstm_unet.sh \
...
echo "Monitor with:"
echo "  squeue -u \$USER"
echo "  tail -f PART1_multi_temporal_experiments/outputs/logs/slurm_*.log"
echo ""
echo "Expected output directories:"
if [ "$MODE" = "kfold" ]; then
    for SAMPLING in "${SAMPLINGS[@]}"; do
        for FOLD in $(seq 0 $(($NUM_FOLDS - 1))); do
            echo "  PART1_multi_temporal_experiments/outputs/experiments/exp001_${MODEL}_${SAMPLING}_seed${SEED}_fold${FOLD}/"
        done
    done
else
    for SAMPLING in "${SAMPLINGS[@]}"; do
        for SEED in $SEEDS; do
            echo "  PART1_multi_temporal_experiments/outputs/experiments/exp001_${MODEL}_${SAMPLING}_seed${SEED}/"
        done
    done
fi
echo "=========================================="
