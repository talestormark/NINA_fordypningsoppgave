#!/usr/bin/env python3
"""
M5: baseline transfer check.

Compares two RQ1 alternative architectures (Pool-7 T=7 and LSTM-2-lite) to
EarlyFusion T=2 on the clamped 260-tile 9-band Sentinel-2 dataset. Both
alternatives use existing trained checkpoints (no new training). The test
evaluation produces ensemble per-tile IoU for 40 held-out tiles. This script
runs the paired permutation tests.

Output: experiments/annotation_efficiency/outputs/statistical_tests/m5_baseline_transfer.json
"""
...
OUTPUT_PATH = AE_DIR / "outputs" / "statistical_tests" / "m5_baseline_transfer.json"
...
ANCHOR = ("EarlyFusion T=2 (A3_s2_9band)", EXP_BASE / "A3_s2_9band" / "test_results.json")

COMPARISONS = [
    {"label": "Pool-7 vs EarlyFusion",      "exp": EXP_BASE / "A3_s2_9band_t7" / "test_results.json"},
    {"label": "LSTM-2-lite vs EarlyFusion", "exp": EXP_BASE / "LSTM7lite_sanity" / "test_results.json"},
]
...
def main():
    ...
    print()
    print(f"{'Comparison':<32} {'n':>3} {'a%':>7} {'b%':>7} {'Δpp':>7} {'95% CI':>20} {'p_perm':>8} {'p_adj':>8} {'Cliff δ':>10}")
    print("-" * 110)
    for r in results:
