...
python experiments/exp2_input_representation/scripts/modeling/evaluate_test_set.py \
...
for fold in 0 1 2 3 4; do
    PRED_DIR="experiments/exp2_input_representation/outputs/experiments/${EXPERIMENT}_fold${fold}/predictions"
    if [ -d "$PRED_DIR" ]; then
        echo "  Fold ${fold}: ${PRED_DIR}"
        python experiments/exp2_input_representation/scripts/analysis/boundary_metrics.py \
...
ENS_DIR="experiments/exp2_input_representation/outputs/experiments/${EXPERIMENT}/ensemble_predictions"
if [ -d "$ENS_DIR" ]; then
